...
#h4[Position update] Agents a claims-maker reaches are pulled toward its pole with a strength set by its _depth_ $alpha_X$ (how hard it pushes those it reaches) and by how far the person still has to travel: $I^D_i (t) = alpha_D g^D_i (t)(1 - b_i (t))$ for the entrepreneur and $I^C_i (t) = alpha_C g^C_i (t)(1 + b_i (t))$ for its opponent. The second factor shrinks as an agent approaches the pole, so a claims-maker acting alone moves each person a fixed _fraction_ of the remaining way and can never push anyone past the end of the scale. Positions then update as
